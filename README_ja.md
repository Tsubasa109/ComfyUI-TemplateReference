...
- 画像をアップロードして画像参照ブロックでプレビューできます。
- 各ブロックをリストとしてJSON ファイルとして保存・読み込みできます。
- Output ドロップダウンから特定のブロックを選択し選択したテキストまたは画像を、ほかの ComfyUI ノードへ出力できます。
- ブロックをドラッグして並べ替えできます。
- ノード UI 上でブロックの折りたたみ、展開、リサイズ、コピー、削除ができます。
- ユーザーが作成したJSONデータはComfyUI の user ディレクトリに保存されます。

プロンプトテンプレート機能:
- プロンプトテンプレートを作成し各ブロックごとにプロンプトのタイトルとプロンプトテキストを設定できます。
- 各ブロックのプロンプトテンプレートには画像をアップロードして画像参照でプレビューできます。
- 各ブロックをリストとしてJSON ファイルとして保存・読み込みできます。
- Output ドロップダウンから特定のブロックのプロンプトテンプレートを選択し選択したプロンプトを、ほかの ComfyUI ノードへ出力できます。
- ブロックをドラッグして並べ替えできます。
- ノード UI 上でブロックの折りたたみ、展開、リサイズ、コピー、削除ができます。
- ユーザーが作成したJSONデータはComfyUI の user ディレクトリに保存されます。
